...
def linear_schedule(initial_value: float) -> Callable[[float], float]:
    """
    학습 진행률(progress_remaining)에 따라 학습률을 선형적으로 줄임.
    1.0 (시작) -> 0.0 (끝)
    """
...
if TRAIN_MODE:
    ...
    policy_kwargs = dict(
        net_arch=dict(pi=[256, 256, 256], vf=[256, 256, 256]), # 뇌 용량도 늘림 (64->128->256 / layer 수 2->3)
        log_std_init=0.0 
    )
    ...
    # 3. 학습 시작 (log_interval=10으로 설정하여 10번 업데이트마다 로그 출력)
    try:
    ...
    try:
        ...
        # 이동 평균(Moving Average) 계산 (그래프 부드럽게)
        window = 50
        if len(y) > window:
        ...
        plt.plot(x, y, alpha=0.3, label='Raw Reward') # 흐린 원본 데이터
        plt.plot(x_smooth, y_smooth, color='red', label='Moving Average') # 진한 이동평균
