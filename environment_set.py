...
class BilliardEnv(gym.Env):
    metadata = {'render_modes': ['human', 'rgb_array'], 'render_fps': 120} # FPS를 60 -> 120으로 올려서 시각적으로도 빠르게
    ...
    def step(self, action):
        ...
        # 보상 계산
        reward = self._calculate_reward(self.start_red_dist, self.min_red_dist)
    ...
    def _calculate_reward(self, start_dist, end_dist):
        reward = -0.1 
        ...
        if self.current_shot == 1:
            # 초구: 빨간공 기준 개선도
            imp = math.sqrt(start_dist) - math.sqrt(end_dist)
            improvement = imp
        # else:
        #     # 초구 이후 : 둘 중 더 많이 가까워진 쪽
        #     imp_y = math.sqrt(s_y) - math.sqrt(m_y)
        #     imp_r = math.sqrt(s_r) - math.sqrt(m_r)
        #     improvement = max(0.0, max(imp_y, imp_r))
            
        # 거리 보상: 100픽셀 다가가면 1점
        distance_score = improvement * 0.01

        # # 거리 보상 -> 공 못 맞춰도 근처로 치면 reward 더 줌
        # dist_sq = min(self.cue_ball.position.get_dist_sqrd(self.obj_ball1.position), self.cue_ball.position.get_dist_sqrd(self.obj_ball2.position))
        
        # # 멀리 있어도 보상을 좀 더 느끼게 분모 조정
        # distance_score = 50000.0 / (50000.0 + dist_sq) 
    ...
    def render(self):
        ...
        # FPS를 높여서(또는 제한을 풀어서) 화면을 더 빨리 돌림
        # 0이면 제한 없음(최고 속도), 120이면 2배속
        self.clock.tick(120) 
